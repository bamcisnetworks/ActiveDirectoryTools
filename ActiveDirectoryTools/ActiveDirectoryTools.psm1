...
Function Get-ADPrincipalDetails {
	...
	Process {
        ...
        if ($Result.Identity -ne $null)
        {
			...
			{
				[Microsoft.ActiveDirectory.Management.ADGroup[]]$Groups = Get-ADNestedGroupMembership -Identity $Result.Identity.DistinguishedName -Domain $Result.Domain.DNSRoot @CredSplat
...
Function Get-ADNestedGroupMembership {
	<#
		...
		.DESCRIPTION
			The Get-ADNestedGroupMembership gets all nested group membership of an AD principal. The principal can be a user, computer, or group.

        .EXAMPLE
			Get-ADNestedGroupMembership -Identity Administrator

	        Gets all group membership for the Administrator account.

		.EXAMPLE
			Get-ADNestedGroupMembership -Identity "Domain Admins -Domain root
